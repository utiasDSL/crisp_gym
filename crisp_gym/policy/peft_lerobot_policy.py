...
@register_policy("peft_lerobot_policy")
class PeftLerobotPolicy(Policy):
...
def inference_worker(
    ...
    try:
        ...
        logger.info("Get the peft policy.")
        peft_wrapper_policy = PeftWrapperPolicy(policy=policy)
        peft_policy = PeftModel.from_pretrained(peft_wrapper_policy, peft_weight_path, is_trainable=False, autocast_adapter_dtype=False)

        logger.info(f"[Inference] {peft_policy}")
