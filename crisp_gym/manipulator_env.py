...
class ManipulatorBaseEnv(gym.Env):
    ...
    def create_action_struct_for_env(self) -> ctypes.Structure:
        """Create a ctypes structure for the actions of the environment.

        Returns:
            ctypes.Structure: The action structure.
        """

        class ActionStruct(ctypes.Structure):
            _fields_ = (
                [
                    ("action.cartesian", ctypes.c_float * 6),
                    ("action.gripper", ctypes.c_float * 1),
                ]
                if self.ctrl_type is ControlType.CARTESIAN
                else [
                    ("action.joint", ctypes.c_float * self.config.robot_config.num_joints()),
                    ("action.gripper", ctypes.c_float * 1),
                ]
            )

            def __str__(self) -> str:
                return str({field[0]: getattr(self, field[0]) for field in self._fields_})

        return ActionStruct

    def create_obs_struct_for_env(self) -> ctypes.Structure:
        """Create a ctypes structure for the observations of the environment.

        Returns:
            ctypes.Structure: The observation structure.
        """

        class ObservationStruct(ctypes.Structure):
            _fields_ = (
                [
                    (
                        f"observation.image.image_{camera.config.camera_name}",
                        ctypes.c_uint8
                        * (camera.config.resolution[0] * camera.config.resolution[1] * 3),
                    )  # type: ignore
                    for camera in self.cameras
                ]
                + [
                    ("observation.state.cartesian", ctypes.c_float * 6),
                    ("observation.state.gripper", ctypes.c_float * 1),
                    (
                        "observation.state.joint",
                        ctypes.c_float * self.config.robot_config.num_joints(),
                    ),
                ]
                + [
                    (
                        f"observation.state.sensor_{sensor.config.name}",
                        ctypes.c_float * np.prod(sensor.config.shape),
                    )  # type: ignore
                    for sensor in self.sensors
                    if hasattr(sensor, "config") and hasattr(sensor.config, "shape")
                ]
